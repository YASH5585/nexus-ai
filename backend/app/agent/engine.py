...
from app.services.local_fallback import local_generator
...
class AutonomousAgent:
    def __init__(
        self,
        openai_service: OpenAIService | HuggingFaceService,
        ...
        for attempt in range(1, self._config.max_retries + 1):
            ...
            if attempt == 1:
                try:
                    ...
                    code = local_generator.generate_code(prompt)
            else:
                reasoning.append(
                    ReasoningStep(
                ...
                try:
                    ...
                    repaired, explanation = local_generator.repair_code(
                        code=code,
                        errors=[e.message for e in errors],
                        prompt=prompt,
                    )
                    ...
                    repair = RepairAction(
                        reason=explanation,
                        code_change=repaired,
                        confidence=0.6,
                        expected_outcome="Local fallback repair applied",
                    )
